...
async function handleApagarMilitar(interaction, ceobDb) {
    try {
        ...
        console.error(`Erro ao apagar militar:`, error.message);
        console.error(`Detalhe PG:`, error.detail, `| Constraint:`, error.constraint, `| Tabela:`, error.table);
        try {
            if (interaction.deferred || interaction.replied) {
                await interaction.editReply(`❌ Ocorreu um erro interno ao apagar o militar: ${error.message}`);
            } else {
                await interaction.reply({ content: '❌ Ocorreu um erro interno ao apagar o militar.', ephemeral: true });
            }
        } catch (replyErr) {
